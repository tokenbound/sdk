			...
			const mintedIds = receipt.logs
				.filter(
					(log) =>
						...
						zora721.proxyContractAddress.toLowerCase(),
				)
				.map((log) => {
					try {
				...
				.filter(
					(event): event is NonNullable<typeof event> =>
						...
						// biome-ignore lint/suspicious/noExplicitAny: event args are contract-specific
						(event.args as any)?.to?.toLowerCase() ===
							ANVIL_USER_0.toLowerCase(),
				)
				// biome-ignore lint/suspicious/noExplicitAny: event args are contract-specific
