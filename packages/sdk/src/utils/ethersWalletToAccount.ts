...
export const ethersWalletToAccount = (wallet: EthersWallet) => {
  if (!wallet.address && !wallet._address) {
    throw new Error('Ethers wallet must have an address')
  }
  // const addr: `0x${string}` = (wallet.address ?? wallet._address) as `0x${string}`

  return toAccount({
    // address: wallet.address as Address,
    address: (wallet.address ?? wallet._address) as `0x${string}`,
    async signMessage({ message }) {
      return (await wallet.signMessage(toBytes(message as any))) as Hash
